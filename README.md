...
2. Нажмите на Нуру на карте. Через панель района добавьте **«Школа и детсад»**, **«Поликлиника»**, **«Освещение и камеры»**, затем городскую **«Платформу обращений»**. Для пятого решения выберите **«Чистое топливо»** в Сарыарке. При необходимости откройте полный список мер в панели.
...
4. Нажмите **«Сохранить как вариант»**, затем **«Применить замену»** в блоке улучшения. Сравнение покажет, какие показатели выиграли, а какие ухудшились. Варианты доступны в шапке; параметры `plan` и `vs` позволяют передать сравнение ссылкой.
...
Нужны **Git, Node.js 22+ и npm**. Последняя проверка выполнена на Node.js 25.8.2. Из нового каталога:
...
Последний проверенный код: **`da0dd85`**, включён в `main` через [PR #30](https://github.com/BAITC-Hacks/hack-5caf5332-stroit-ai/pull/30). Результаты на **23.09.2026**:
...
Playwright сам запускает Vite на порту **5178**. API и платные AI-вызовы в браузерных проверках заменены фикстурами. Это проверка поведения приложения, а не доступности провайдеров. Живые OpenAI/Jev-вызовы в этом прогоне не выполнялись. Мобильные форматы не входили в проверку.
...
Настройки находятся в [wrangler.jsonc](akim/ui/wrangler.jsonc). Один Worker обслуживает интерфейс и `/api/*`. Нужны аккаунт Cloudflare, KV-пространство и серверные секреты. Для другого аккаунта создайте собственное KV и замените ID привязки `CACHE` в конфигурации.
