...
Проект команды **Stroit AI** для кейса **Sim Astana** на **HackAlem AI 2026**. Пользователь распределяет бюджет между городскими инициативами, видит изменения на карте и получает итоговый **Astana Quality of Life Score** с объяснением сильных сторон, рисков и компромиссов. Главное в проекте — симуляция горожан: 5 000 AI-жителей реагируют на каждый план, а карта жалоб из Threads привязывает модель к реальным проблемам районов.

<h3 align="center"><a href="https://sim-astana.diaskhalniyasov.workers.dev">Try this out →</a></h3>

[Запустить локально](#быстрый-запуск) · [Пройти сценарий проверки](#демонстрация-за-пять-минут) · [Техническая документация](akim/ui/README.md) · [Условия кейса](akim/tz.md)
...
| AI-аким | Совет по замене, автопилот и подбор плана по цели: приоритеты, резерв бюджета, защита районов, обязательные и запрещённые меры. Предложение применяется по действию пользователя. На итоговом листе совет озвучивает пиксельный аким: во время проверки он рассказывает о каждом шаге, затем произносит вывод. |
| Реакция жителей | 5 000 синтетических жителей-агентов, распределённых по районам пропорционально реальному населению; один агент ≈ население города ÷ 5 000 (по живым данным stat.gov). Шесть профилей интересов в каждом районе. Расчёт поддержки и объяснение того, кому помогает план. |
